...
public class AuroraClusterMonitorClusterCheckerTest {
  ...
  static {
    ...
    CLUSTER_SERVERS.put(testServer1, new TestServerMonitor(SERVER_MONITOR_SCHEDULER, testServer1));
    AuroraServer testServer2 = new AuroraServer("host2", new Properties());
    CLUSTER_SERVERS.put(testServer2, new TestServerMonitor(SERVER_MONITOR_SCHEDULER, testServer2));
  }
  ...
  private static class TestServerMonitor extends ServerMonitor {
    protected TestServerMonitor(SubmitterScheduler scheduler, AuroraServer server) {
      super(server, new TestReschedulingOperation(scheduler));
    }
